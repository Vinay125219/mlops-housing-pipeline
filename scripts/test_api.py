...
def test_iris_api():
    """Test the iris classification API"""
    print("Testing Iris API...")
    ...
    try:
        ...
        if response.status_code == 200:
            print("✅ Iris API health check passed")
        else:
            print(f"❌ Iris API health check failed: {response.status_code}")
            return False
        
        # Test metrics endpoint
        response = requests.get("http://localhost:8001/metrics", timeout=10)
        if response.status_code == 200:
            print("✅ Iris API metrics endpoint working")
        else:
            print(f"❌ Iris API metrics failed: {response.status_code}")
            return False
        ...
        if response.status_code == 200:
            ...
            if "predicted_class" in result:
                print(f"✅ Iris API prediction working: {result['predicted_class']}")
            else:
                print("❌ Iris API prediction response format incorrect")
                return False
        else:
            print(f"❌ Iris API prediction failed: {response.status_code}")
            return False
